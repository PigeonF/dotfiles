systemConfig:
{ config, pkgs, lib, ... }: {
  imports = [ ../../../../users/common ../../../../dotfiles/bash ];

  home = {
    packages = builtins.attrValues {
      inherit (pkgs) gdb gitlab-ci-local nodejs rr valgrind;
    };
    ...
    file = {
      ...
      ".gitlab-ci-local/ca-bundle.crt" = {
        text = (builtins.readFile "${pkgs.cacert}/etc/ssl/certs/ca-bundle.crt")
          + "\n"
      ...
      ".gitlab-ci-local/.env" = {
        text = let
          ...
          variables = lib.strings.concatStringsSep " "
            [ "SSL_CERT_FILE=/etc/ssl/certs/nixbox-bundle.crt" ];
        in ''
          PRIVILEGED=true
          VOLUME="${volumes}"
          VARIABLE="${variables}"
