{ config, pkgs, inputs, ... }:
let inherit (inputs.nixpkgs-networking.lib) ipv4;
in {
  ...
  services.dockerRegistry = let
    docker0 =
      ipv4.cidrToFirstUsableIp config.virtualisation.docker.daemon.settings.bip;
  in {
    enable = true;
  ...
  system.activationScripts = {
    ...
    docker-registry-certs-d.text = ''
      ...
      cp -L ${
        ./minica.pem
      } /etc/docker/certs.d/${config.nixbox.registryHost}/ca.crt
    '';
  };
}
